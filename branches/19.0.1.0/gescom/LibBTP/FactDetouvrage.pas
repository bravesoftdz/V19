...
type
  ...
  TChpModif = (TchpQte,TChpPerte,TchpRendement);
    ...
    public
      ...
      property TOBCatalogu : TOB read fTOBCatalogu write fTOBCatalogu;
      //
      ...
      procedure RechercheCodeArticle (Acol,Arow : integer; TOBL : TOB; CodeArticle : string; RefRecherche : string);
...
uses factureBTP,factOuvrage,UtilPGI,FactArticle,facture,FactTOB,
     ...
     ParamSoc,UspecifPOC, FactFormule, UFonctionsCBP, Ent1,
...
procedure TModifSousDetail.RechercheCodeArticle(Acol,Arow : integer; TOBL: TOB;CodeArticle: string; RefRecherche : string);
...
var RechArt,EtatRech : T_RechArt ;
    OkArt,Trouvok   : Boolean ;
    st1 : string;
    TOBArt,TobTemp : TOB;
    TOBPiece,TOBAA,TOBCata,TOBArtRef : TOB;
    sWhere, stChamps, StatutArt, TypeRef : string;
    QQ : TQuery;
    MtPaf : double;
    ActionArt : TActionFiche;
    OneFour : string;
    FromLiaisonCatalogue : Boolean;
begin
  FromLiaisonCatalogue := false;
  Libelle := '';
  TypeArticle := '';
  OkArt:=False ;
  //-----------------------------------------------------------------
  ...
  //-----------------------------------------------------------------
  //
  if (TFFacture(XX).TheOptionCat.GestionActive) and (TFFacture(XX).TheCurrentCatalog.Fournisseur<>'') then
  begin
    if TrouveArticleViaCatalogue (Codearticle,TOBArt,TOBCata,TOBCatalogu,TOBArticles,
                                  TFFacture(XX).TheOptionCat,TFFacture(XX).TheCurrentCatalog,
                                  TFFacture(XX).CleDoc.DatePiece,TFFacture(XX).IsVenteAchat,
                                  TFFacture(XX).Action) then
    begin
      RechArt := traOk;
      FromLiaisonCatalogue := True;
    end;
  end else
  begin
  ...
  case RechArt of
    ...
    traAucun : begin
      ...
      //-----------------------------------------------------------------
      if ((not TFFacture(XX).TheOptionCat.GestionActive) or (TFFacture(XX).TheCurrentCatalog.Fournisseur='')) then
      begin
      ...
      begin
        ...
        OneFour := 'GCA_TIERS='+Trim(TFFacture(XX).TheCurrentCatalog.Fournisseur)+';';
        StChamps := OneFour+'GCA_REFERENCE=' + Trim(Copy(CodeArticle, 1, 18)) + ';XX_WHERE=GCA_DATESUP >= "'+USDATETIME(TFFacture(XX).Cledoc.DatePiece)+'"';
        CodeArticle := AGLLanceFiche('GC', 'GCMULCATALOG_RECH', '', '', StChamps);
        CodeArticle := READTOKENST(CodeArticle);
        if TrouveArticleViaCatalogue (Codearticle,TOBArt,TOBCata,TOBCatalogu,TOBArticles,
                                      TFFacture(XX).TheOptionCat,TFFacture(XX).TheCurrentCatalog,
                                      TFFacture(XX).CleDoc.DatePiece,TFFacture(XX).IsVenteAchat,
                                      TFFacture(XX).Action) then
        begin
          if TFFacture(XX).TheOptionCat.CreatArtFromCat then
          begin
            CodeArticle := TOBART.GetString('GA_CODEARTICLE');
          end;
          RechArt := traOk;
          FromLiaisonCatalogue := True;
          okArt := true;
  ...
  if (OkArt) and (not FromLiaisonCatalogue) then
  begin
    //--------------------------------------------------------------------------
  ...
  end else if (OkArt) and (FromLiaisonCatalogue) then
  begin
    Article := TOBArt.GetValue('GA_ARTICLE');
    if CodeArticle = '' then CodeArticle := TOBArt.GetValue('GA_CODEARTICLE');
    if Libelle = '' then Libelle := TOBArt.GetValue('GA_LIBELLE');
    if TOBCata <> nil then
    begin
//      CodeArticle := TOBCata.GetValue('GCA_REFERENCE');
      Libelle := TOBCata.GetValue('GCA_LIBELLE');
      (*
      if TOBCata.GetDouble('GCA_PRIXVENTE') = 0 then TOBArt.SetDouble('GA_PAHT',TOBCata.GetDouble('GCA_PRIXBASE'))
                                                else TOBArt.SetDouble('GA_PAHT',TOBCata.GetDouble('GCA_PRIXVENTE'));
      *)
    end;
  end else
  begin
  ...
  if (not FromLiaisonCataLogue) then
  begin
    TOBArt.free;
  end else
  begin
    if (TOBArt<>nil) and (not OkArt) then TOBARt.free;
    if (TOBCata<>nil) and (TOBCata.getString('GCA_REFERENCE')='') then TOBCata.free;
  end;
  Result := OkArt;
end;
