...
procedure SetVisible(FieldName : string; FF : TForm);
var cField : TControl;
begin
  ...
  begin
    ...
    if Assigned(cField) then cField.Visible := (VH_GC.BTCODESPECIF = '002');
  end;
...
procedure GereOptGestionCata(CC : TControl);
var FF : TForm;
    Gere, Gere1 : boolean;
begin
  FF := GetLaForm(CC);
  if ExisteNam('SO_BCATOPTACTIF', FF) then
  begin
    if TCheckBox(GetFromNam('SO_BCATOPTACTIF',FF)).Checked then
    begin
      TCheckBox(GetFromNam('SO_BCATOPTAFFECTART',FF)).enabled := true;
      if TCheckBox(GetFromNam('SO_BCATOPTAFFECTART',FF)).Checked then
      begin
        THEdit(GetFromNam('SO_BCATOPTARTGEN', FF)).text := '';
        THEdit(GetFromNam('SO_BCATOPTARTGEN', FF)).enabled := false;
        TCheckBox(GetFromNam('SO_BCATOPTVOIRART',FF)).enabled := true;
        TCheckBox(GetFromNam('SO_BCATOPTFAMCPTA',FF)).enabled := true;
      end else
      begin
        TCheckBox(GetFromNam('SO_BCATOPTFAMCPTA',FF)).enabled := false;
        TCheckBox(GetFromNam('SO_BCATOPTVOIRART',FF)).Checked := false;
        TCheckBox(GetFromNam('SO_BCATOPTVOIRART',FF)).enabled := false;
        TCheckBox(GetFromNam('SO_BCATOPTFAMCPTA',FF)).enabled := false;
        THEdit(GetFromNam('SO_BCATOPTARTGEN', FF)).enabled := true;
      end;
    end else
    begin
      TCheckBox(GetFromNam('SO_BCATOPTAFFECTART',FF)).Checked := false;
      TCheckBox(GetFromNam('SO_BCATOPTAFFECTART',FF)).enabled := false;
      THEdit(GetFromNam('SO_BCATOPTARTGEN', FF)).text := '';
      THEdit(GetFromNam('SO_BCATOPTARTGEN', FF)).enabled := false;
      TCheckBox(GetFromNam('SO_BCATOPTFAMCPTA',FF)).checked := false;
      TCheckBox(GetFromNam('SO_BCATOPTFAMCPTA',FF)).enabled := false;
      TCheckBox(GetFromNam('SO_BCATOPTVOIRART',FF)).Checked := false;
      TCheckBox(GetFromNam('SO_BCATOPTVOIRART',FF)).enabled := false;
    end;
...
{ GC_JTR_GC15601_Fin }
  ...
  else if (NamChpExit = 'SO_BCATOPTAFFECTART') or (NamChpExit = 'SO_BCATOPTACTIF') then
  begin
    GereOptGestionCata (CC);
  end
{$IFDEF GCGC}
...
begin
  ...
  begin
    SetVisible('SCO_EXPXML',FF);
    SetVisible('SO_EXPXMLDIR',FF);
    SetVisible('SO_EXPXMLDE', FF);
    SetVisible('SO_EXPXMLA', FF);
    SetVisible('SO_EXPXMLDOSSIERCPTA', FF);
    SetVisible('SO_EXPXMLCLIENTCPTA', FF);
  end;
