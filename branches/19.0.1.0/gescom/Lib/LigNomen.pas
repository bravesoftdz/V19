...
{$ENDIF}
    ...
    procedure AlimenteCurrentCatalog;
    procedure ChangeModeRech(Sender: TObject);
    procedure RemplitChoixRecherche;
...
begin
CurrentCatalog := TCatalog.Create;
OnValidation := false;
...
{$ENDIF}
// -- BOI
if (TFfacture(Xp).VerrouilleBOI) then
begin
  BintegreExcel.Visible := false;
  QTEDETAIL.Enabled := false;
end;
  RemplitChoixRecherche;
  TMODERECH.OnChange := ChangeModeRech;
// --
...
begin
	QTEDETAIL.Enabled := (Action<>taConsult) and (not (TFfacture(Xp).VerrouilleBOI));
...
Case Key of
    ...
    VK_INSERT : BEGIN
                ...
                if TFfacture(Xp).VerrouilleBOI then exit;
{$IFDEF BTP}
...
//--------------------------------------------------------------------------
  ...
  begin
    St1 := TOB(G_NLIG.Objects[SGN_NumLig, ARow]).getString('BLO_CODEARTICLE');
  end;
...
begin
  ...
  //-----------------------------------------------------------------
  if ((not TFFacture(XP).TheOptionCat.GestionActive) or (CurrentCatalog.Fournisseur='')) then
  begin
    TOBArt := TOB.Create('ARTICLE',nil,-1);
  ...
  begin
    if TrouveArticleViaCatalogue (CodeArt,TOBART,TOBCata,nil,nil,TFFacture(XP).TheOptionCat,
                                  CurrentCatalog,TFFacture(XP).CleDoc.DatePiece,TFFacture(XP).IsVenteAchat,
                                  TFFacture(XP).Action) then
    begin
      RechArt := Traok;
      TypeArticle := TOBArt.GetValue('GA_TYPEARTICLE');
      FromCatalogue := true;
  ...
  traAucun : begin
    ...
    begin
      StChamps := 'GCA_TIERS='+Trim(CurrentCatalog.Fournisseur)+';GCA_REFERENCE=' + Trim(Copy(G_NLIG.Cells[SGN_Comp, ARow], 1, 18)) + ';XX_WHERE=' + sWhere;
      CodeArt := AGLLanceFiche('GC', 'GCMULCATALOG_RECH', '', '', StChamps);
      if CodeArt <> '' then
      begin
        CodeArt := ReadTokenST(CodeArt);
        if TrouveArticleViaCatalogue (CodeArt,TOBART,TOBCata,nil,nil,TFFacture(XP).TheOptionCat,
                                      CurrentCatalog,TFFacture(XP).CleDoc.DatePiece,TFFacture(XP).IsVenteAchat,
                                      TFFacture(XP).Action) then
        begin
          ART.text := TOBART.getString('GA_CODEARTICLE');
        end;
      end;
    end;
    if ART.Text <>'' then
  ...
  if (TOBART<>nil) and (Tobart.GetValue('GA_FERME') = 'X') then
  begin
  ...
  begin
    ...
    if Ouvrage then RenseigneTOBOuv(TOB(G_NLIG.Objects[SGN_NumLig, ARow]),TobArt,Libelle)
               else RenseigneTOBLN(TOB(G_NLIG.Objects[SGN_NumLig, ARow]),TobArt,TOBInfo);
    ...
    begin
      if gestionHt then
        ...
        G_NLIG.cells[SGN_PVHT,Arow] := StrF00(TOB(G_NLIG.Objects[SGN_NumLig, ARow]).getvalue('BLO_PUTTCDEV'),V_PGI.OkdecP);
    ...
    begin
      if TOBCata <> nil then
      begin
        TobTemp.PutValue(prefixe+'_LIBELLE', TOBCata.GetValue('GCA_LIBELLE'));
        G_NLIG.Cells[SGN_Lib, ARow] := TOBCata.GetValue('GCA_LIBELLE');
        if not TFFacture(XP).TheOptionCat.CreatArtFromCat then
        begin
          TobTemp.PutValue(prefixe+'_CODEARTICLE', TOBCata.GetValue('GCA_REFERENCE'));
          G_NLIG.Cells[SGN_Comp, ARow] := TOBCata.GetValue('GCA_REFERENCE');
        end;
      end;
    end;
  end else
  ...
  if TOBArt <> nil then TOBArt.free;
...
Function TFLigNomen.ZoneAccessible ( ACol,ARow : Longint) : boolean ;
  ...
  function IsZoneCalcul(Acol : integer) : boolean;
  begin
    result := (Acol = SGN_FOURN) or (Acol = SGN_PAHT) or (Acol = SGN_Qte) or (Acol = SGN_Comp) or (Acol = SGN_PREST) or (Acol = SGN_PVHT) or (Acol = SGN_PERTE) or (Acol = SGN_RENDEMENT) or (Acol = SGN_QTESAIS) or (Acol = SGN_TPS);
  end;
...
BEGIN
  result := true;
  if (TFfacture(Xp).VerrouilleBOI) and (IsZoneCalcul(Acol)) then
  begin
    Result := false;
    exit;
  end;
...
begin
  ...
  begin
    ...
    if ART.Text <> '' then
    begin
      G_NLIG.Cells[G_NLIG.Col,G_NLIG.Row]:= copy(ART.Text, 0, Length(ART.Text) - 1);
    end;
  end else
  begin
    sWhere := 'GCA_DATESUP >= "'+USDATETIME(TFFacture(XP).CleDoc.datePiece)+'"';
    StChamps := 'GCA_TIERS='+Trim(CurrentCatalog.Fournisseur)+';GCA_REFERENCE=' + Trim(Copy(G_NLIG.Cells[G_NLIG.Col,G_NLIG.Row], 1, 18)) + ';XX_WHERE=' + sWhere;
    CodeArt := AGLLanceFiche('GC', 'GCMULCATALOG_RECH', '', '', StChamps);
    if CodeArt <> '' then
    begin
      CodeARt := ReadTokenST(CodeArt);
      G_NLIG.Cells[G_NLIG.Col,G_NLIG.Row]:= CodeArt;
    end;
  end;
...
procedure TFLigNomen.AlimenteCurrentCatalog;
var QQ :TQuery;
begin
  if TMODERECH.ItemIndex >= 0 then
  begin
    CurrentCatalog.Fournisseur := TMODERECH.Values [TMODERECH.ItemIndex];
    CurrentCatalog.Libelle := TMODERECH.Items [TMODERECH.ItemIndex];
    CurrentCatalog.prefixe := '';
    if CurrentCatalog.Fournisseur <> '' then
    begin
      QQ := OpenSql('SELECT BT1_PREFIXEART FROM TIERS LEFT JOIN BTIERS ON T_AUXILIAIRE=BT1_AUXILIAIRE WHERE T_NATUREAUXI="FOU" AND T_TIERS="'+CurrentCatalog.Fournisseur+'"',true,1,'',true);
      if not QQ.eof then
      begin
        CurrentCatalog.prefixe := QQ.fields[0].AsString;
      end;
      ferme(QQ);
      if (CurrentCatalog.prefixe = '') and (TFFacture(XP).TheOptionCat.CreatArtFromCat) then PGIInfo('ATENTION : Le prefixe fournisseur n''est pas renseigné');
    end;
  end;
end;


procedure TFLigNomen.RemplitChoixRecherche;
var II : integer;
begin
  TMODERECH.Clear;
  for II := 0 to TFFacture(Xp).TMODERECH.Items.Count -1 do
  begin
    TMODERECH.Items.Insert(II,TFFacture(Xp).TMODERECH.items[II]);
    TMODERECH.Values.Insert(II,TFFacture(Xp).TMODERECH.values[II]);
  end;
  TMODERECH.ItemIndex :=TFFACture(Xp).TMODERECH.ItemIndex;
  AlimenteCurrentCatalog;
end;


procedure TFLigNomen.ChangeModeRech(Sender: TObject);
begin
  AlimenteCurrentCatalog;
end;
end.
