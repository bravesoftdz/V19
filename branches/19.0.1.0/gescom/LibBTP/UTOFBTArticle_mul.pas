...
Inherited;
  ...
  //
  if (ecran.Name = 'BTARTICLE_RECH') Or (ecran.Name = 'BTPREST_RECH') or (ecran.Name = 'BTARTSTOCK_MUL') then
  Begin
...
// Ajout BRL : Sélection sur tarif article et sous-famille tarif
  ...
  begin
    ...
    else if (copy(ecran.Name,1,14) = 'BTARTICLE_RECH') Or (copy(ecran.Name,1,14) = 'BTARTSTOCK_MUL') then
    begin
      TT := st;
      repeat
...
begin
  ...
  if FamilleN1 <> nil then
  begin
    // Modified by f.vautrain 30/04/2019 15:19:09 - FS#3646 - ESPACS - La gestions des familles/sous-familles non hiérarchique ne fonctionne pas
...
procedure TOF_BTARTICLE_MUL.OnClose;
begin
  inherited;
