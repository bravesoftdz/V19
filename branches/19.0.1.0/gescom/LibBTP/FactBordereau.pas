...
function IsExisteLigneBordereau(TOBPiece: TOB): boolean;
function GetTOBAFromBordereau (TOBArticles: TOB; Tiers,RefArt : string; DateDocument : TdateTime) : TOB;
...
begin
  ...
  Requete := 'SELECT ##TOP 1## *,BDETETUDE.* FROM LIGNE '+
             'LEFT JOIN BDETETUDE ON '+
             'BDE_NATUREPIECEG=GL_NATUREPIECEG AND BDE_SOUCHE=GL_SOUCHE AND '+
             'BDE_NUMERO=GL_NUMERO AND BDE_INDICEG=GL_INDICEG '+
             'WHERE '+
             '(GL_NATUREPIECEG="BBO") AND (GL_REFARTTIERS="'+RefArt+'") AND '+
             'BDE_CLIENT="'+Tiers+'" AND BDE_NATUREAUXI="CLI" AND '+
             '("'+USDATETIME(DateDocument)+'" BETWEEN BDE_DATEDEPART AND BDE_DATEFIN)';
  TRY
  ...
  begin
    TOBA := TOBArticles.FindFirst (['GA_ARTICLE'],[TOBLOC.detail[0].getString('GL_ARTICLE')],true);
    if TOBA = nil then
    begin
      QQ := OpenSql ('SELECT A.*,AC.*,N.BNP_TYPERESSOURCE FROM ARTICLE A '+
                     'LEFT JOIN NATUREPREST N ON N.BNP_NATUREPRES=A.GA_NATUREPRES '+
                     'LEFT JOIN ARTICLECOMPL AC ON AC.GA2_ARTICLE=A.GA_ARTICLE WHERE A.GA_ARTICLE="'+TOBLOC.detail[0].getString('GL_ARTICLe')+'"',true,-1, '', True);
      if not QQ.eof then
      begin
    ...
    if TOBA <> nil then
    begin
      TObA.seTDouble('GA_PVHT',TOBLoc.detail[0].GetDouble('GL_PUHTDEV'));
      if TObA.GetDouble('GA_DPR') <> 0 then
      begin
        TOBA.setDouble('GA_COEFCALCHT', ARRONDI(TOBLoc.detail[0].GetDouble('GL_PUHTDEV')/TObA.GetDouble('GA_DPR'),4));
      end else TOBA.setDouble('GA_COEFCALCHT', 0);
      if not TOBA.fieldExists('DEJA_CALCULE') then TObA.AddChampSupValeur('DEJA_CALCULE','-',false); 
      TOBA.SetString('DEJA CALCULE','X');
    end;
  end;
  TOBLoc.Free;
end;
