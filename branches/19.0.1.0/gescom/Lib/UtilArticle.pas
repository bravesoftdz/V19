...
Type TChangeCodeArt = class
  ...
  ToptionCatArticles = class (TObject)
    GestionActive : boolean;
    CreatArtFromCat : Boolean;
    ArtEtalon : String;
    OpenFormAfterCreat : Boolean;
    FamComptaToUse : string;
    ArtGenerique : string;
  public
    constructor create;
  end;
...
constructor ToptionCatArticles.create;
begin
  GestionActive := GetParamSocSecur('SO_BCATOPTACTIF',false);
  ArtEtalon := GetParamSocSecur('SO_BCATOPTARTETALON','');
  CreatArtFromCat := GetParamSocSecur('SO_BCATOPTAFFECTART',false);
  OpenFormAfterCreat := GetParamSocSecur('SO_BCATOPTVOIRART',false);
  FamComptaToUse := GetParamSocSecur('SO_BCATOPTFAMCPTA','');
  ArtGenerique := GetParamSocSecur('SO_BCATOPTARTGEN','');
  if (GestionActive) and (((CreatArtFromCat) and (ArtEtalon='')) or ((not CreatArtFromCat) and (ArtGenerique=''))) then
  begin
    PGIInfo('Merci de renseigner les paramètres de Relation Catalogue/article#13#10 Désactivation de cette gestion');
    GestionActive := false;
  end;
end;
{$IFDEF GPAO}
